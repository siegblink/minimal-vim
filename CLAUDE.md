...
- **The light selection blue is `#96bce6` (1.77:1 on the base), and selection surfaces need *luminance* contrast, not chroma.** Its predecessor `#bfd6ee` had 1.33:1 — all of its contrast in blue tint, which wide-gamut panels flatter and sRGB monitors flatten. (The *total* invisibility of Visual mode that prompted this was a different bug — the night-owl focus hijack below — but the contrast floor is real and kept.) `test-theme.lua` asserts a 1.5:1 Visual-vs-Normal luminance floor in both modes. The value is one colour across the whole stack — theme.lua (`visual`, `lg_selected`, `lg_cherry_bg`), the chezmoi theme script (lazygit `sel`/`cherry_bg`, fzf `bg+`) and Ghostty's light `selection-background` — with `scripts/test-lazygit-theme.lua`'s STANDALONE table pinning the parity. When changing it: the theme script's `sync` gate checks mode/file-existence, **not content**, so regenerated followers keep stale colours — delete `~/.config/lazygit/config.yml` and `~/.config/fzf/theme`, then `theme sync` (required on each machine after pulling the dotfiles change).

## Known Quirks

- **night-owl focus autocmds hijack Visual (deleted by theme.lua)**: night-owl.nvim's `after/plugin/autocmds.lua` registers global `FocusGained`/`FocusLost` autocmds running `hi! link Visual @nowl.visual.{active,inactive}` (dim-on-blur). Its guard — `:colorscheme` output == "night-owl" at sourcing time — always passes in this config: night-owl's `setup()` sets `g:colors_name` before the async first `sync()` lands a scheme. A link *shadows* a group's own attributes, so every alt-tab undid the pinned Visual: fine-looking in dark (the link target exists and matches the pin), **invisible selection in light** (`highlight clear` wiped the `@nowl` groups; Visual resolved to nothing while `nvim_get_hl` still showed the pinned bg in the raw definition — assert with `link = false`, and test rendering with a focus cycle, not just group values). `strip_visual_focus_hijack()` in `theme.lua` deletes these autocmds on every `highlights()` run; `test-theme.lua` registers an identical hijacker and asserts a focus cycle can no longer break either mode. Cost: night-owl's dim-on-blur selection is gone in dark too — intentional, theme.lua owns Visual. Headless/pty repros must send `\x1b[O`/`\x1b[I` (focus out/in) to trigger it — every real-world report involved alt-tabbing (screen recorder, browser), which is why it looked machine- or pull-correlated when it was neither.
...
- **Python LSP (python-lsp-server, NOT in git)**: pylsp is the Mason package `python-lsp-server` (`:MasonInstall python-lsp-server`) — an external binary installed once per machine; without it Python buffers show a pylsp spawn error. On Debian/Ubuntu the install itself fails with Mason's opaque `spawn: python3 failed with exit code 1` until `sudo apt install python3.X-venv` matching the system Python (e.g. `python3.12-venv`): Mason installs pypi packages into a venv it creates via `python3 -m venv`, and Debian/Ubuntu strip ensurepip/venv out of base `python3`. The real error message only surfaces when running `python3 -m venv <dir>` by hand. macOS Pythons bundle venv, so the same install works there with no extra step.
...
- **Visual selection under the noice cmdline (`noice.lua`)**: noice's ext_cmdline drops the still-active visual selection from any buffer repaint while the cmdline is open (folke/noice.nvim#478, closed not-planned). With a `V…G` selection past the viewport the repaint is guaranteed — `:` scrolls the view back to the range start — so the whole selection rendered un-highlighted. `noice.lua` re-applies the tracked region as `vim.hl.range` extmarks (Visual group, so theme.lua's pinned colour applies) for the duration of the cmdline. The region MUST be tracked while visual mode is live: `:` emits `ModeChanged V:n` (never `V:c`), and by `CmdlineEnter` both the cursor and `getpos("v")` have already jumped to the range start — reading marks or the anchor at cmdline time returns garbage. `<Esc>` also emits `V:n`; the two are distinguished by whether `CmdlineEnter` follows within the same input batch (`left_visual` flag with a scheduled reset). Note `'<,'>` appearing in the cmdline and the view jumping to the selection start are both normal Neovim behaviour (they happen in `nvim --clean` too); only the missing highlight was the bug.
